...
def aging_route(
    ...
    if limited:
        return limited
    try:
        engine._validate_business_exists(business_id)
...
def calculate_route(
    ...
    try:
        ...
        result = engine.calculate(business_id, month, parsed_policy)
        return result
...
def export_route(
    ...
    try:
        ...
        if format.lower() == "json":
            return result
        csv_text = engine.export_csv(business_id, result)
...
def policy_route(payload: ProvisionPolicyUpdateRequest):
    try:
        updated = engine.update_policy(payload.business_id, payload.policy)
        return {"business_id": payload.business_id, "policy": updated}
    except LookupError:
        return _error_response(404, "not_found", "Business not found")
    except ValueError as exc:
        return _policy_or_month_error(exc)
